...
wordnode::wordnode(char *in)    //一下是通过一个哈希表来构建单词池
{
	copystr(word, in);
	next = NULL;
}
...
{
	char word_array[MAX_WORD_NUM + 1] = { '\0' };        //从文件中读入单词，进行建池
	if (filetype == EASY)
	{
...
void word_pool::rand_word(char **&wordptr, int startnum, int stopnum)//该函数可从单词池中随机4挑选单词
{
	srand(time(NULL));
	int lab[52];
