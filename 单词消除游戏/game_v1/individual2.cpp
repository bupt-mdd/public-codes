...
void individual::show_result(individual *Arrayptr, int num)
{
	cout << setw(PROPER_NUM / 4) << setiosflags(ios::left) << setfill(' ') << "名次"
		...
		<< setw(PROPER_NUM / 2) << setiosflags(ios::left) << setfill(' ') << "闯关数(出题)" << endl;
...
int individual::init_operate(int command, int role, string player_inf, string adm_inf)//通过该函数调用注册和登陆函数
{
	if (role == SET_QUE_MAN)
	{
		if (REGISTER == command)
