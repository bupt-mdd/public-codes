...
void individual::change_username(string cur_inf)//修改姓名
{
...
void individual::change_password(string cur_inf)//修改姓名
{
	string copy_account, copy_name, copy_code;
	int copy_rank, copy_experience, copy_num;
	fstream AccountFile(cur_inf, ios::in | ios::out);
...
int individual::user_register(string cur_inf)//实现玩家注册。
{
	system("cls");
...
int individual::user_login(string cur_inf)//玩家登陆
{
	system("cls");
	string account;
	int copy_rank, copy_experience, copy_num;
...
void individual::change(int role, string player_inf, string adm_inf, int pass_num,int cur_experience)
{                       //实现经验，等级等的修改，
	if (role == PLAYER)
	{
		string copy_account, copy_name, copy_code;
...
void individual::search_All_individual(int role, string player_inf, string  adm_inf)
{              //查询输出所有的玩家信息
	string copy_account, copy_name, copy_code;
	int copy_rank, copy_experience, copy_num;
...
void individual::search_individual(int role, string player_inf, string  adm_inf)
{                            //按属性查询玩家信息
	int in_command;
...
int individual::account_search(char find_str[], string cur_inf)//按账号查询
{
...
int individual::name_search(char find_str[], string cur_inf)//按姓名查询
{
	char copy_account[PROPER_NUM] = { '\0' },
		copy_name[PROPER_NUM] = { '\0' },
		copy_code[PROPER_NUM] = { '\0' };
...
int individual::rank_search(int find_int, string cur_inf)//按等级查询
{
	string copy_account, copy_name, copy_code;
	int copy_rank, copy_experience, copy_num;
...
int individual::exp_search(int find_int, string cur_inf)//按经验查询
{
	string copy_account, copy_name, copy_code;
	int copy_rank, copy_experience, copy_num;
...
int individual::num_search(int find_int, string cur_inf)//按出题数和闯关数查询
{
	string copy_account, copy_name,  copy_code;
	int copy_rank,  copy_experience,  copy_num;
