...
from kvm.lib import KVM_Benchmark_lib
import json

class KVM_Benchmark_Main(object):
    ...
    def __init__(self, rule=None, server='10.108.114.24', port=22, user='root', password='ics123'):
    ...
    def run(self):
        if self.connectOK:
            #　如果可以连接ssh　，　执行基线检测函数
            kvmLib = KVM_Benchmark_lib(self.ssh, self.ssh1, self.rule)
            self.result = kvmLib.getResult()
            pass
        else:
            pass
    ...
    def connect(self):
        try:
            ...
            self.ssh1.connect(hostname='10.108.113.63',
                             port=22,
                             username='ics',
                             password='123456')
...
if __name__ == "__main__":
    kvmBenchmark = KVM_Benchmark_Main(server='10.108.114.24')
    result = kvmBenchmark.getResult()
    resultJson = kvmBenchmark.getJsonResult()
    print(result)
    print(resultJson)
