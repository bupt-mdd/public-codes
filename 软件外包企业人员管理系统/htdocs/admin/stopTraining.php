...
?>
	<div class="container">
    <div class="row">
	<form action="stopTraining.php" method="POST" target=_self>
        <table class="table table-hover table-bordered">
		<caption><h3 class="center">该培训活动即将结束，请对信息进行最后的确认或修改</h3></caption>
		<tbody>
			<tr>
...
else if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	$query="update trainingInfTable set isEnd='yes',startTime='$startTime',endTime='$endTime',trainingContent='$trainingContent' ".
	"where trainingName='$trainingName'";
	$result=mysqli_query($conn,$query)
			or die('Error querying database1'.mysqli_error());
	
	$query="SELECT userId FROM userToTraining where trainingName='$trainingName'";
	$result=mysqli_query($conn,$query)
			or die('Error querying database1'.mysqli_error());
	while($row=mysqli_fetch_array($result))
	{
		$userId=$row["userId"];
		$query1="update userInfTable set isBusy=0 where userId='$userId'";
...
?>
    ...
    <div class="row">
        <table class="table table-hover table-bordered">
		<caption><h3 class="center">该培训活动已经结束，相关信息如下</h3></caption>
		<tbody>
			<tr>
