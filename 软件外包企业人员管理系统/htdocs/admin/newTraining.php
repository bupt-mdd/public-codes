...
?>
	<div class="container">
    <div class="row">
	<form action="newTraining.php" method="POST" target=_self>
        <table class="table table-hover table-bordered">
		<caption><h1 class="center">培训活动相关信息录入</h1></caption>
		<tbody>
			<tr>
			<td class="col-sm-4">培训名称：<label style="color:#FF0000">*</label></td>
			<td class="col-sm-8"><input type="text" placeholder="please input name" id="trainingName" class="form-control" name="trainingName" required></td>
			</tr>
			<tr>
			...
			<td><textarea placeholder="please input awards" id="trainingContent" class="form-control" name="trainingContent" required></textarea></td>
			</tr>
			<tr>
...
else if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	$query="SELECT count(trainingName) as number FROM trainingInfTable where trainingName='$trainingName'";
...
?>
    ...
    <div class="row">
		...
		<tbody>
			...
			<td class="col-sm-8"><label style="color:#FF0000">该培训活动已经存在，不可重复添加，请选择编辑操作</label></td>
			</tr>
		</tbody>
		</table>
	</div>
...
<?php
	...
	{
		$query="insert into trainingInfTable (trainingName,startTime,endTime,trainingContent,isEnd) ".
			"values ('$trainingName','$startTime','$endTime','$trainingContent','no')";
		$result=mysqli_query($conn,$query)
...
?>
	...
	<form action="newResume.php" method="POST" target=_self>
        <table class="table table-hover table-bordered">
		<caption><h1 class="center">培训信息录入成功</h1></caption>
		<tbody>
			<tr>
