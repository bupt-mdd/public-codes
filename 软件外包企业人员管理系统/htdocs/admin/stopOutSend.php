...
?>
	<div class="container">
    <div class="row">
	<form action="stopOutSend.php" method="POST" target=_self>
        <table class="table table-hover table-bordered">
		<caption><h3 class="center">此次外派即将结束，请对相关信息进行再次确认或最后修改</h3></caption>
		<tbody>
			<tr>
...
else if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	$query="update outSendInfTable set isEnd='yes',startTime='$startTime',endTime='$endTime',outSendContent='$outSendContent' ".
	"where outSendName='$outSendName'";
	$result=mysqli_query($conn,$query)
			or die('Error querying database1'.mysqli_error());
	
	$query="SELECT userId FROM userToOutSend where outSendName='$outSendName'";
	$result=mysqli_query($conn,$query)
			or die('Error querying database1'.mysqli_error());
...
?>
    ...
    <div class="row">
        <table class="table table-hover table-bordered">
		<caption><h3 class="center">该外派已经结束，相关信息如下</h3></caption>
		<tbody>
			<tr>
